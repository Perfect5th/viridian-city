import Character from './character.js';
import Background from './base/background.js';

// Constants
// ====================================

const WIDTH = 160;
const HEIGHT = 144;

const CANVAS_CSS = "position: absolute; top: 0; left: 0; z-index: 1";

const DEFAULTS = {
  SCALING: 1,
  TICK_SPEED: 42,
};

const KEYCODES = {
  RIGHT: 39,
  LEFT:  37,
  UP:    38,
  DOWN:  40,
  A:     65,
};

// Class Definitions
// ====================================

class VCGame {
  /*
   * VCGame is new VCGame(
   *   Element,
   *   PlayerCharacter,
   *   Level,
   *   {
   *     scaling: Number,
   *     tickSpeed: Number,
   *   }
   * )
   *
   * Interp. A Viridian City Game with a PlayerCharacter (pc) and a starting
   *   Level on a screen scaled by scaling and tick speed of tickSpeed.
   *
   *   The game is rendered on an HTML5 canvas within the given Element.
   *
   * Usage:
   *  - VCGame.start(): start the game
   *  - VCGame.stop(): stop the game
   */

  constructor(screen, pc, level, options={}) {
    this.screen    = screen;
    this.pc        = pc;
    this.level     = level;
    this.scaling   = options.scaling || DEFAULTS.SCALING;
    this.tickSpeed = options.tickSpeed || DEFAULTS.TICK_SPEED;
  }

  tick() {
    /*
     * Updates the world state every this.tickSpeed milliseconds.
     */

    this.level.update();
    this.render();
  }

  awaitAssets() {
    /*
     * -> Promise
     * Produces a Promise that resolves when all required assets have loaded.
     */


    return Promise.all([
      this.level.hasLoaded(),
      this.pc.hasLoaded(),
      ...this.level.npcs.map(npc => npc.hasLoaded()),
    ]);
  }

  render() {
    /*
     * Renders the game world to the HTML5 canvas.
     */

    this.awaitAssets()
      .then(() => this.level.render(
        this.bgContext,
        this.pc.position,
      ))
      .then(() => Promise.all(
        this.level.npcs.map(npc => npc.render(
          this.charContext,
          this.level.tileSize,
          this.pc.position,
        ))
      ))
      .then(() => this.pc.render(
        this.charContext,
        this.level.tileSize,
      ))
      .catch(error => console.error(error));
  }

  handleKey(kc) {
    /*
     * KeyCode ->
     * Runs the appropriate keyevent handler for the given key.
     */

    switch (kc) {
      case KEYCODES.RIGHT:
        return this.pc.moveRight(this.level);
      case KEYCODES.LEFT:
        return this.pc.moveLeft(this.level);
      case KEYCODES.UP:
        return this.pc.moveUp(this.level);
      case KEYCODES.DOWN:
        return this.pc.moveDown(this.level);
      case KEYCODES.A:
        return this.pc.interact(this.level);
      default:
        return;
    }
  }

  start() {
    /*
     * Starts the game by starting the clock and creating the keydown listeners.
     */

    document.addEventListener('keydown', e => this.handleKey(e.keyCode));

    this.backgroundCanvas = document.createElement('canvas');
    this.backgroundCanvas.style.cssText = CANVAS_CSS;
    this.backgroundCanvas.width = WIDTH * this.scaling;
    this.backgroundCanvas.height = HEIGHT * this.scaling;

    this.characterCanvas = this.backgroundCanvas.cloneNode();
    this.characterCanvas.style.zIndex = 2;

    this.screen.style.position = 'relative';

    this.screen.appendChild(this.backgroundCanvas);
    this.screen.appendChild(this.characterCanvas);

    this.bgContext = this.backgroundCanvas.getContext('2d');
    this.charContext = this.characterCanvas.getContext('2d');

    this.clock = setInterval(() => this.tick(), this.tickSpeed);
  }

  stop() {
    clearInterval(this.clock);
  }

};

export default VCGame;
