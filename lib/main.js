...
class VCGame {
  ...
  */
  ...
  getControlCallback(name) {
    // Looks up and returns the control function for the given name.
    const controlCallbacks = {
      moveUp: this.playerCharacter.move('up'),
      moveLeft: this.playerCharacter.move('left'),
      moveDown: this.playerCharacter.move('down'),
      moveRight: this.playerCharacter.move('right'),
      interact: this.playerCharacter.interact,
    };

    let callback = controlCallbacks[name];
    if (typeof callback === 'undefined')
      return (() => {});

    return callback.bind(this.playerCharacter);
  }

  executeControl(cbInput) {
    // Executes cbInput, which may be an array, string, or function.
    if (cbInput instanceof Array)
      cbInput.forEach(cb => this.executeControl(cb));
    else if (typeof cbInput === 'string') {
      let callback = this.getControlCallback(cbInput);
      callback();
    } else if (cbInput instanceof Function)
      cbInput();
  }

  handleKeyDown(e) {
    // Checks if keydown events are in the controlSchema, then executes the
    // corresponding binding.

    if (!Object.keys(this.controlSchema).includes(e.key))
      return;

    e.preventDefault();

    this.executeControl(this.controlSchema[e.key]);
  }

  bindControlSchema(schema) {
    // Binds a control schema to the game, allowing control of the player
    // character.
