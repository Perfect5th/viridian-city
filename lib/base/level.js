...
import Background from '../canvas/image/background.js';
...
class Level extends Renderable {
  ...
  get currentCoords() {
    // Returns the current [x, y] as pixel values on the canvas.
    const [x, y] = this.game.playerCharacter.currentTile;
  ...
  get currentWindow() {
    // Returns the currently visible range of the level background image.
    const start = this.currentCoords.map(c => c - 4 * this.game.tileSize);
    const end = [
      this.game.levelLayer.width,
      this.game.levelLayer.height,
    ].map(d => d / this.game.scaling);
