...
import { COORDS } from './characters.js';
...
const EMPTY_LEVEL = Array(10).fill(Array(10).fill(new Tile(true)))
  .map(arr => arr.slice());
...
class Level {
  ...
  constructor(uri, w, h, tileSize, tileSet, npcs=[]) {
    /*
    ...
    *   (pixels), a square tile size (pixels), a set of possibly
    *   interactible tiles, and a set of NPCs.
    */
  ...
  getTile(pos) {
    ...
    try {
      return this.tileSet[pos[COORDS.X]][pos[COORDS.Y]];
    } catch (e) {
      return false;
    }
  }
  ...
  update() {
    /*
     * Level -> Level
     * Produces next level given current level state
     */

    this.tileSet.map(tileCol => tileCol.map(tile => tile.occupied = false));
    this.npcs.map(npc => this.getTile(npc.position).occupied = true);
  }
